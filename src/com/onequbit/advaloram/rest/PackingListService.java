...
public class PackingListService {
	...
	public static Response createPackingList(@Context HttpServletRequest request, 
		...
		try {
			...
			result = new JSONObject();
			result = PackingListDao.createOrUpdatePackingList((long) -1, inputStreamArray, userId);
		} catch (Exception e) {
			result = new JSONObject();
			result.put(Application.RESULT, Application.ERROR);
			result.put(Application.ERROR_MESSAGE, e.getMessage());
	...
	public static Response updatePackingList(@Context HttpServletRequest request, 
		...
		try {
			result = new JSONObject();
			result = PackingListDao.createOrUpdatePackingList(id, inputStreamArray, (long) -1);	//not saving. So no userId required
		} catch (Exception e) {
			result = new JSONObject();
			result.put(Application.RESULT, Application.ERROR);
			result.put(Application.ERROR_MESSAGE, e.getMessage());
